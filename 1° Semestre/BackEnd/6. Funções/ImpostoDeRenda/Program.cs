...
static float ImpostoDeRenda(float imposto){
    ...
    else if ((imposto >= 1501) && (imposto <= 3500))
    {
        ...
        Console.WriteLine($"Você vai pagar 20% de imposto. O valor a pagar é {imposto}");
    }
    else if ((imposto >= 3501) && (imposto <= 6000))
    {
        imposto = (imposto/100) * 25;
        Console.WriteLine($"Você vai pagar 25% de imposto. O valor a pagar é {imposto}");
    }
    else
    {
        imposto = (imposto/100) * 35;
        Console.WriteLine($"Você vai pagar 35%. O valor a pagar é {imposto}");
    }
